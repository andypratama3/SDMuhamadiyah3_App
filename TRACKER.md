...
### Sesi 6 — iOS native: lokasi (CLLocationManager) + SSL pinning (Ktor Darwin)
- **DeviceLocation iOS aktual** (sebelumnya stub: selalu `NEED_PERMISSION`, `getCurrentLocation` selalu gagal → check-in GPS absensi guru mati di iPhone): ditulis ulang native `IosLocationCoordinator` (`NSObject(), CLLocationManagerDelegateProtocol`) — `manager.delegate`, `desiredAccuracy = kCLLocationAccuracyBest`.
- `permissionState()` → 4-state enum via `CLAuthorizationStatus`; `LOCATION_SERVICES_OFF` dari `CLLocationManager.locationServicesEnabled()`. Permintaan izin memakai `requestWhenInUseAuthorization()`; callback izin via `locationManagerDidChangeAuthorization` (iOS 14+) + retained `didChangeAuthorizationStatus:` (iOS <14, `@Suppress("DEPRECATION")`).
- `getCurrentLocation`/`watch` via `didUpdateLocations` (one-shot stop setelah deliver, watch continuous), `didFailWithError` untuk error; timeout 20s & guard izin dipertahankan. Akses koordinat memakai `coordinate.useContents { latitude/longitude }` (CValue struct) + `horizontalAccuracy` — pola diverifikasi dari contoh kompilasi-sukses (StackOverflow/Rapido).
- Verifikasi API dari **binding lokal** `~/.konan/.../platform.CoreLocation` (selector `&locationManagerDidChangeAuthorization:`, `#locationManager:didUpdateLocations:`, `requestWhenInUseAuthorization`, `start/stopUpdatingLocation`, tipe `CLAuthorizationStatus`) — semua tersedia di K/N 2.4.10.
- **SSL pinning iOS** (sebelumnya no-op + komentar ATS): `SslPinning.ios.kt` → cast `DarwinClientEngineConfig` + `CertificatePinner.Builder().add("app.sdmuhammadiyah3smd.com", pin)` untuk tiap pin `CertificatePins.pins` (`sha256/...`) + `config.handleChallenge(builder.build())`. API diverifikasi dari docs resmi `api.ktor.io/.../io.ktor.client.engine.darwin.certificates` (Ktor 3.1.2). Guard `pins.isEmpty()`; paritas perilaku dengan `SslPinning.android.kt` (OkHttp).
- Info.plist tetap: ATS exception domain `app.sdmuhammadiyah3smd.com` (TLS≥1.2, FS, no arbitrary loads); `NSLocationWhenInUseUsageDescription` + `NSLocationAlwaysAndWhenInUseUsageDescription` sudah terpasang.
- Kode saja — **tidak build/run**; verifikasi statis (grep symbol + baca ulang file) PASS. Tanpa compile-check (tanpa gradle): hanya simbol dari binding lokal + docs resmi yang dipakai.

### Sesi 7 — Fix sisa layar belum-audit (SplashScreen, badge guru)
- **SplashScreen footer SCHOOL_NAME** (L342): teks `heroContent.copy(alpha = isDark?0.5:0.35)` melayang di `BottomCenter` — di light theme zona bawah gradient = `primaryContainer(#D6E3FF)` terang → putih 35% ≈ 1.1:1 tak terbaca (pelanggaran kontrak heroContent-alpha harus di atas navy). → solid `colorScheme.onPrimaryContainer` (#001B3D di light, #D6E3FF di dark — kontras kuat di kedua tema). `isDark`/`heroContent` masih terpakai (orb/progress/title) — tidak ada var mati.
- **AbsensiSaya CheckInStatusBadge** (L241-245): badge "Alpha" `errorContainer` / "Terlambat" `tertiaryContainer` → sejajar kontrak status helper: `statusSuccessColor` (Tepat Waktu), `statusWarningColor` (Terlambat), `statusDangerColor` (Alpha) — pola fill `statColor@0.12` + teks solid (pola StatusChip diterima §6), seragam dengan KehadiranSiswa/Legenda.
- **KegiatanProgramScreen** (L373): ikon "Aktivitas terakhir" `primary@0.3` → `onSurfaceMuted` (seragam token ikon).
- Braces 3 file = 0; tanpa import baru (helpers via `theme.*` yang sudah ada). Audit loop-3 (sweep `surfaceTint`/`copy(alpha)`/divider/tab-gold/error-status) → **satu-satunya bug UI tersisa sudah difix di sini**.

### Sesi 8 — Audit menyeluruh 31 screen + komponen (7 agen paralel) → "Perbaikin semuannya"
- **HIGH (2 file, compile breaker)**: `.sp` dipakai tapi `import androidx.compose.ui.unit.sp` tidak ada di `TeacherHomeScreen.kt` (letterSpacing L148/189) & `AbsensiSayaScreen.kt` (L141) → import `sp` ditambah (kebetulan token `dp` di AbsensiSaya mati — dihapus).
- **MED design-system**:
  - `Sdm3CommonPatterns.kt` `Sdm3ErrorBanner`: border `error@0.3` → solid `error`; ikon & teks `error` → `onErrorContainer` (white/cream on errorContainer, kontras kuat). `Sdm3IconBadge` default: bg `primaryContainer@0.3` → solid `primaryContainer`; border `primary@0.2` → `outline`.
  - `Sdm3Button.kt` outlined: border `finalContentColor@0.2` → `colorScheme.outline`.
  - `Sdm3AdaptiveNav.kt` drawer unselected: ikon `onSurfaceVariant@0.7` → `ProductSchoolTheme.colors.onSurfaceMuted`.
- **MED auth**: PilihAnak selected-state gold → `primary`: border animasi (L193), ring foto (L237 — unselected → `outline`), badge check (L290 + icon `onPrimary`), spinner (L66); `PilihAnakBottomSheet.kt` badge check → `primary`.
- **MED nilai/rapor**: `NilaiRaporScreen.kt` chip semester border (selected `secondary`→`primary`, unselected `primary@0.2`→`outline`), ring badge L544→`outline`; `DetailNilaiMapelScreen.kt` persen+progress Formatif `secondary`→`primary`; `VerifikasiQrRaporScreen.kt` frame scan `primary@0.1`→`outline`.
- **MED notifikasi/profil**: `NotifikasiScreen.kt` L390 & `PengumumanSekolahScreen.kt` L195 border `primary@0.1`→`outline`; `ProfilAkunScreen.kt` 3× `HorizontalDivider primary@0.05`→`outline` (L199/219/434), 2× ring avatar→`outline` (L481/575); `PengaturanNotifikasiScreen.kt` divider→`outline`, switch checked track `secondary`→`primary`.
- **MED guru**: `TeacherHomeScreen.kt` badge "Absensi Saya" ikon `secondary`+bg `secondaryContainer@0.35` (≈2:1, hambar) → `primary`+`primaryContainer@0.35` (sejajar TeacherClassroomCard).
- **Ring ikon / border alpha tersisa (lolos agen, tertangkap sweep): 6 file** → `outline`: `PembayaranBerhasil` L190, `DetailBuktiBayar` L419, `ProsesPembayaran` L198, `PilihMetodeBayar` L248 (unselected), `NilaiRapor` L544, `KegiatanProgram` L228. Sweep final `BorderStroke/border/borderColor primary|secondary.copy` → **0**.
- **Dead imports dihapus (±24 di 22 file)**: TeacherHome (`background`, `RoundedCornerShape`), AbsensiSaya (`dp`), design-system (`Modifiers·Composable`, `ScreenScaffold·SnackbarHost`, `Sdm3Card·Spacer`, `Sdm3GlassCard·Box`+`Alignment`, `Sdm3CommonPatterns·height`), Onboarding (`ArrowForward`), PilihAnak (`TextAlign`), PreviewRapor (`Color`), VerifikasiQr (`Color`), Notifikasi (`Sdm3EmptyState`, `EmptyStateStyle`), PengaturanNotifikasi (`Icons`, `Color`), GuruAbsensi (`Icons`), HomeScreen (`GridCells`, `LazyVerticalGrid`, `MutableInteractionSource`, `KeyboardArrowRight`), PembayaranBerhasil (`CircleShape`, `setValue`, `Color`), ProsesPembayaran (`Phone`, `setValue`, `Color`), PilihMetodeBayar (`CircleShape`, `Color`, val `liquidGlassSurface`/`liquidGlassBorder`). Koreksi: `getValue` ×6 DIPERTAHANKAN (delegate `val x by …`, operator tak muncul tekstual); `FlowRow` GuruAbsensi TEPAT dipakai (L156) — jangan dihapus; `setValue` aman dihapus (delegate setempat `val … by`).
- **Gradient bayar**: `PaymentUiComponents.kt` L124 `inversePrimary@0.4` → `0.35` (sesuai kontrak §6).
- Verifikasi akhir (statis): brace seluruh repo **NONE** tak seimbang; dead-import macro `\b` → **0** (sisa hanya operator getValue/Composable yang terpakai); sisa `primary/secondary.copy(alpha)` = fill/glow/track/glow dekoratif (kategori diterima §6).
- Tidak difix (tercatat §6): StatusChip `status@0.10..0.15`+teks solid ≈3.3:1 (perlu token container-status baru di Color.kt — rekomendasi), `Badge.kt` error/onError (pair Material sah), banner sukses VerifikasiOtp (transien), fills `primaryContainer@0.3..0.4` container tint.
